...
- **Not every real property is in the TSL model.** The tested fan's temperature has no TSL entry at all — `get_tsl()` never returns a `code: "temperature"` property, so there's no documented id to `read()` or match against. It's still pushed locally like anything else, though: on this fan, field id `21` turned out to carry it, found by connecting with `on_update` logging every raw field and cross-referencing an unrecognized id's value against the device's own display (see [landbook-ha#27](https://github.com/zackwag/landbook-ha/issues/27) for how this was diagnosed). If you need a property the TSL doesn't mention, watching raw pushes is the only way to find its id — and don't assume it's the same id on a different product.
